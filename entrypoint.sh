...
mkdir -p "$PGDATA"
chown -R andavar:andavar "$PGDATA"

# Ensure PGDATA exists and is empty before initdb if it doesn't exist
if [ ! -d "$PGDATA/base" ]; then
    echo "Initializing database..."
    su -s /bin/bash andavar -c "initdb -D '$PGDATA' --auth-local=trust --auth-host=trust"
fi
...
su -s /bin/bash andavar -c "postgres -D '$PGDATA' -h 127.0.0.1 -k /tmp > /tmp/postgres.log 2>&1" &
PG_PID=$!
...
psql -h 127.0.0.1 -p 5432 -U andavar -d postgres -c "ALTER USER andavar WITH PASSWORD 'andavar';" || true
psql -h 127.0.0.1 -p 5432 -U andavar -d postgres -c "SELECT 1 FROM pg_database WHERE datname = 'andavar'" | grep -q 1 || \
    psql -h 127.0.0.1 -p 5432 -U andavar -d postgres -c "CREATE DATABASE andavar OWNER andavar;"

# Execute CMD passed to docker run
cmd=$(printf "%q " "$@")
exec su -s /bin/bash andavar -c "$cmd"
